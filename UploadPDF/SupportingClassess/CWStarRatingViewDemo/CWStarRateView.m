...
#define FOREGROUND_STAR_IMAGE_NAME @"rate-star" //"b27_icon_star_yellow"
...
- (void)setScorePercent:(CGFloat)scroePercent {
   // if (_scorePercent == scroePercent) {
    if ([[NSString stringWithFormat:@"%f",_scorePercent] isEqualToString: [NSString stringWithFormat:@"%f",scroePercent]]) {
    ...
    NSLog(@"scroePercent...%.02f",scroePercent);
    
//    let appDelegate = UIApplication.sharedApplication().delegate as! AppDelegate
//    let aVariable = appDelegate.baseurl
    
    StrRating=[NSString stringWithFormat:@"%.f",(scroePercent*10 )/2];
    NSLog(@"StrRating...%@",StrRating);
